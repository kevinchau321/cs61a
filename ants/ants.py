...
class Ant(Insect):
	...
	def can_contain(self, ant):
		if self.container: # True if bodyguard
			if self.ant == None:
				return not ant.container
			return False # the bodyguard has an ant
		return False
...
class ThrowerAnt(Ant):
	...
	def nearest_bee(self, hive):
		...
		while current_place != hive:
			if current_place.bees: ##current place has bees
				if current_distance <= self.max_range and current_distance >= self.min_range: 
...
class FireAnt(Ant):
	...
	def reduce_armor(self, amount):
		self.armor -= amount
		bees_copy = [b for b in self.place.bees]
		if self.armor <= 0:
...
class QueenAnt(ScubaThrower):
	...
	def __init__(self):
		...
		self.queen_inst = QueenAnt.num_queens
		self.doubled_ants = []

	def action(self, colony):
		"""A queen ant throws a leaf, but also doubles the damage of ants
		in her tunnel.  Impostor queens do only one thing: die."""
		if self.queen_inst != 1:
			Insect.reduce_armor(self, self.armor)
			QueenAnt.num_queens -= 1 # For the sake of keeping the definition
		else:
			colony.queen = QueenPlace(colony.queen, self.place)
			entrance_to_check = self.place.entrance
			while entrance_to_check != None:
				cur_ant = entrance_to_check.ant
				if cur_ant is not None and cur_ant not in self.doubled_ants:
					if isinstance(cur_ant, BodyguardAnt):
						if cur_ant.ant != None:
							cur_ant.ant.damage *= 2
					cur_ant.damage *= 2
					self.doubled_ants += [cur_ant]
				entrance_to_check = entrance_to_check.entrance
			exit_to_check = self.place.exit
			while exit_to_check != None:
				cur_ant = exit_to_check.ant
				if cur_ant is not None and cur_ant not in self.doubled_ants:
					if isinstance(cur_ant, BodyguardAnt):
						if cur_ant.ant != None:
							cur_ant.ant.damage *= 2
					cur_ant.damage *= 2
					self.doubled_ants += [cur_ant]
				exit_to_check = exit_to_check.exit
...
def apply_effect(effect, bee, duration):
	...
	def new_action(colony):
		nonlocal duration
		# bee.action = effect(old_action)
		if duration > 0:
			duration -= 1
			return effect(old_action)(colony) #bee.action(colony)
		# bee.action = old_action
		return old_action(colony) # bee.action(colony)
