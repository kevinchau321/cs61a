...
class MinimalAmount(Amount):
    ...
    """
    def __init__(self, nickels, pennies):
        self.nickels = (5*nickels + 1*pennies) // 5
        self.pennies = (5*nickels + 1*pennies) % 5
...
def apply(operator_name, shape):
    ...
    12
    """
    apply_implementations = {('area', Square): lambda shape: shape.side**2,
                             ('perimeter', Square): lambda shape:shape.side*4,
                             ('area', Rect): lambda shape:shape.width*shape.height,
                             ('perimeter', Rect): lambda shape:shape.width*2+shape.height*2}
...
def tree_to_ordered_sequence(s):
    ...
    def tree_to_list(s):
        ...
        if s == None:
            return
        list1.append(s.entry)
        tree_to_list(s.right)
        tree_to_list(s.left)
    tree_to_list(s)
    def list_to_rlist(list1):
        if list1 == []:
            return Rlist.empty
        smallest = min(list1)
        list1.remove(smallest)
        return Rlist(smallest, list_to_rlist(list1))
    return list_to_rlist(list1)
  

def tien_tree_to_ordered_sequence(s):
    if s == None:
    	return
    if s.left == None and s.right is None:
    	return s.entry
    if s.left is None and s.right is not None:
    	return Rlist(s.entry,tien_tree_to_ordered_sequence(s.right))
    if s.right == None and s.left is not None:
    	return Rlist(tien_tree_to_ordered_sequence(s.left),s.entry)
    return Rlist(tien_tree_to_ordered_sequence(s.left),Rlist(s.entry,tien_tree_to_ordered_sequence(s.right)))
...
def partial_tree(s, n):
    ...
    for _ in range(left_size+1):
        right_branch_set = right_branch_set.rest

    left_branch = partial_tree(s, left_size)
    tree_entry = s.__getitem__(left_size+1)
    right_branch = partial_tree(right_branch_set,right_size)
    

    rest_of_s = right_branch_set
    for _ in range(right_size+1):
        rest_of_s= rest_of_s.rest
    
    return Tree(tree_entry, left_branch, right_branch), rest_of_s
