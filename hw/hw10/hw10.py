...
def check_positive(x):
	...
	if isinstance(x, int) or isinstance(x, float):
		...
		raise TypeError(x + ' is not a number')  #will handle numbers accidentally inputed as strings
...
def interpret_mobile(s):
	...
	for t in ('Branch', 'Weight', 'Mobile'):
		mobile = s[:next_T]+t+s[next_T+1:]
		if interpret_mobile(mobile) is not None:
			return interpret_mobile(mobile)
	return None
