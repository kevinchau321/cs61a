...
def average_sentiments(tweets_by_state):
	...
	for state, tweets in tweets_by_state.items():
		count, state_sent = 0, 0
		for tweet in tweets:
			if has_sentiment(analyze_tweet_sentiment(tweet)):
				state_sent += sentiment_value(analyze_tweet_sentiment(tweet))
				count += 1
		if state_sent == 0:
				averaged_state_sentiments = averaged_state_sentiments
		if count > 0:
				averaged_state_sentiments[state] = state_sent / count
