...
def roll_dice(num_rolls, dice=six_sided):
	...
	while i < num_rolls:
		roll = dice()
		if DEBUG:
			print("Roll number " + str(i+1) + " is " + str(roll))
		total += roll
		if roll == 1:
			rolled_one = True
		i += 1
	if rolled_one:
		if DEBUG:
			print("Rolled 1, Pig out")
		return 1 # Ignore the result of the other rolls
	return total
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
	...
	if num_rolls == 0: #implements free bacon rule
		ones = opponent_score % 10 #extracts ones digit
		tens = opponent_score // 10 #extracts tens digit
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
	...
	while score < GOAL_SCORE and opponent_score < GOAL_SCORE:
		side_number=select_dice(score, opponent_score)
		if who == 0:
			score = score + take_turn(strategy0(score, opponent_score), opponent_score, side_number)
		else:
			opponent_score = opponent_score + take_turn(strategy1(opponent_score, score), score, side_number)
...
def max_scoring_num_rolls(dice=six_sided):
	...
	average_scores=[]
	while k <= 10:
		average_score = make_averaged(roll_dice, 1000)(k, dice)
		average_scores.append(average_score)
		print(str(k) + ' dice scores ' + str(average_score) + ' on average')
		k = k + 1
	i = 1
...
def get_bacon_values(score, opponent_score):
	"""Returns the ones and tens digits of the opponent score, the bacon points
	from using the opponent score, and the resulting score from free bacon """
	ones = opponent_score % 10 #extracts ones digit
	tens = opponent_score // 10 #extracts tens digit
	bacon_points = max(ones, tens) + 1
	return ones, tens, bacon_points, score + bacon_points
...
def bacon_strategy(score, opponent_score):
	...
	if max(ones, tens) + 1 >= BACON_MARGIN:
		return 0
...
def swap_strategy(score, opponent_score):
	...
	5
	"""
	ones, tens, bacon_points, resulting_score = get_bacon_values(score, opponent_score)
	if resulting_score * 2 == opponent_score:
		return 0
	if opponent_score * 2 == resulting_score:
		return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
	...
	Uses the bacon strategy when it is most advantageous to do so, 
	such as when the bacon score will result in an automatic win, 
	a 4 sided dice for the opponent (via hog wild rule), 
	or score swap when the player’s score is less than the opponents.
	Similarly, the strategy also returns 10 as the number of dice to roll 
	when adding only one point (it is highly probably to roll a 1 with this many dice) 
	is advantageous, such as when this will result in the opponent 
	receiving four sided dice or an advantageous score swap. 
	The bacon score is also only used when the player is sufficiently ahead of the opponent, 
	when a riskier move will not be helpful to the player. 
	When the player’s score is in fact less than the opponent’s, 
	a riskier move with higher turn score is used (roll 6 dice)
	"""
	local_bacon_margin = 8
	local_baseline_num_rolls = 5
	ones, tens, bacon_points, resulting_score = get_bacon_values(score, opponent_score)
	if resulting_score >= 100: # regardless if the bacon_points < local_bacon_margin
		return 0
	if (resulting_score + opponent_score) % 7 == 0:
		return 0
	if (score + opponent_score + 1) % 7 == 0 and score > opponent_score + 20:
		return 10

	if score >= opponent_score:
		local_baseline_num_rolls = 4
	else:
		local_baseline_num_rolls = 6

	if (score + opponent_score) % 7 == 0:
		local_baseline_num_rolls = 4 # max average of four-sided dice
	...
	if (score + 1)*2 == opponent_score:
		return 10
	if opponent_score * 2 == resulting_score:
		return local_baseline_num_rolls
	# bacon strategy
	if bacon_points >= local_bacon_margin and score > opponent_score + 10:
		return 0	
	return local_baseline_num_rolls
