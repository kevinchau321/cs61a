...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('# of formal parameters does not match # of values')
        for par in formals:
            frame.bindings[par] = vals.first
            vals = vals.second
        return frame
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        check_form(vals, 2, 2)
        env.bindings[target] = scheme_eval(vals[1], env) ##Need Scheme_eval for problem 8
        return target
...
def do_let_form(vals, env):
    ...
    for bind in bindings:
        if names is nil and values is nil: # both can only be nil together
            names = Pair(bind.first, nil)
            names_start = names
            values = Pair(scheme_eval(bind.second.first, env), nil)
            values_start = values
        else:
            names.second = Pair(bind.first, nil)
            names = names.second
            values.second = Pair(scheme_eval(bind.second.first, env), nil)
            values = values.second
    new_env = env.make_call_frame(names_start, values_start)
...
def check_formals(formals):
    ...
    symbols = []
    k = 0
    while k <= len(formals) - 1:
        if not scheme_symbolp(formals[k]):
            raise SchemeError('Parameter is not a scheme symbol')
        if formals[k] in symbols:
            raise SchemeError('Duplicate symbol')
        symbols.append(formals[k])
        k += 1
