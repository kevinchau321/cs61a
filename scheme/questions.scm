...
(define (list-partitions total max-pieces max-value)
    (cond ((> max-value total) (list-partitions total max-pieces total))
          ((< max-value 1) nil)
          ((> max-pieces 0) 
            (let ((test-w-max (helper (- total max-value) (- max-pieces 1) 
                    max-value (cons max-value nil)))
                  (test-wo-max (list-partitions total max-pieces
                    (- max-value 1))))
                  (append test-w-max test-wo-max))
            )
      )
  )
...
(define (helper total max-pieces max-value parts)
  (cond ((= total 0) (list parts))
        ((< total 0) nil)
        ((and (= max-pieces 0) (> total 0)) nil)
        (else (define (attach lst) (append parts lst))
           (map attach (list-partitions total max-pieces max-value)))
    )
  )
...
(define (flatten deep-list) 
	(let ((new '())) 
		(cond ((null? deep-list) deep-list)
					((pair? (car deep-list)) (append (flatten (car deep-list))
