...
def adv_parse(line):
    ...
    token = tokens.pop(0)
    if token == 'give':
        return 'give', ' '.join(tokens[0:-2]), tokens[-1]
    elif token == 'ask':
        return 'ask', tokens[0], ' '.join(tokens[2:])
...
def adv_eval(exp):
    ...
    >>> place = WerdnasHouse
    >>> adv_eval(adv_parse('give rubber ducky to Wendra'))
    You give Wendra the rubber ducky
    """
    ...
    if is_person(exp):
        ...
        print('You give ' + adv_eval(exp[2]) + ' the ' + adv_eval(exp[1]))
    elif exp[0] == 'ask':
        print('You ask ' + adv_eval(exp[1]) + adv_eval(exp[2]))
    else:
        return adv_apply(exp[0], exp[1])

def adv_apply(operator, operand):
    return adv_eval(operator)(operand)
